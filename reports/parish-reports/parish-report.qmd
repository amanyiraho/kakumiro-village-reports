...
```{r}
library(tidyverse)
parish_selected <-  params$parish
```


# `r parish_selected` parish report
...
data_cleaned <- data |> 
  ...
  filter(section2.q3_parish  == parish_selected) 

number_of_households <- data_cleaned |>  
  group_by(section2.q4_village, section4.q12_hh_number) |> 
  count() |> ungroup() |> 
  count(section2.q4_village)  |> 
  summarise(n = sum(n, na.rm = T))|> pull(n)

data_filtered <- data_cleaned|> 
  filter(section4.Q15_do == "yes")
```

There are `r length(data_filtered$section2.q3_parish)` children in `r parish_selected` parish in our eligible category (4 to 59 months) whose care givers were interviewed.  
...
```{r}
data_zd <- bind_rows(zd_said_no, zd_said_yes_card_no_thigh_no, zd_said_yes_card_yes_dpts_no) |> count(section2.q4_village)


```

There are `r sum(data_zd$n)` Zero dose children in `r parish_selected` parish 
